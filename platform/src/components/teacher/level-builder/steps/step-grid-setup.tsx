      ...
      {isCanvasLayout(config) ? (
        <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
          <div className="grid gap-0 lg:grid-cols-[1fr_1.05fr]">
            <div className="space-y-3 p-5 sm:p-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  No robot grid — white board + yellow strip. Configure content in the Program step.
                </p>
              </div>
              <dl className="grid gap-1.5 text-sm">
                <div className="flex justify-between gap-4 rounded-lg bg-slate-50 px-3 py-2">
                  <dt className="text-slate-500">Strip</dt>
                ...
                <div className="flex justify-between gap-4 rounded-lg bg-slate-50 px-3 py-2">
                  <dt className="text-slate-500">Accepted answers</dt>
                  <dd className="font-medium text-slate-900">
            ...
            <div className="border-t border-slate-100 bg-slate-50/60 p-4 lg:border-l lg:border-t-0">
              <div className="rounded-xl border border-slate-200 bg-white p-4 text-center">
                ...
                <div className="mt-3 flex min-h-[2.5rem] flex-wrap items-center justify-center gap-1.5 rounded-lg bg-amber-100/90 px-2 py-2">
                  {(config.canvasLesson?.stripMode ?? "EMPTY") === "BLANKS"
                    ...
                    : (config.canvasLesson?.stripMode ?? "EMPTY") === "COUNT_ANSWER"
                      ? (
                          <span className="inline-flex items-center gap-1 rounded-lg border border-amber-300 bg-white px-2 py-1 text-sm font-bold tabular-nums text-slate-900">
