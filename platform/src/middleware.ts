...
/**
 * Use `middleware.ts` (not Next.js 16 `proxy.ts`).
 * On Next 16.2 + Turbopack, `src/proxy.ts` makes App Router API routes
 * (including `/api/auth/session`) return HTML 404 → NextAuth CLIENT_FETCH_ERROR.
 */
const UserRole = {
  STUDENT: "STUDENT",
  TEACHER: "TEACHER",
